...
function reload() {
    ...
    if ((kanapReload.length) > (kanapLoad.length)) { // si nouveau LS > LS init. ; ajout d'un nouveau "kanap" par l'utilisateur
...
function getKanapIdApi() {
    ...
    for (let dataKanap of newKanap) { // Parcours NewKanap pour créer coul. et nbr. Et fetch avec l'id
        ...
        fetch(url + `${dataKanap.id}`) //requête "fetch" pour aller chercher les informations dans l'API
            ...
            .then(function(data) { //Récupération de la promise en .json; data est un objet
...
function displayKanap(data) {
    ...
    affichage[i] = //Tableau d'affichage des différents "kanaps" sélectionnés
        ...
        <div class="cart__item__content__settings">
            <div class="cart__item__content__settings__quantity">
                <p>Qté : </p>
                <input onclick="totalDisplayOnClick(${i})" type="number" class="itemQuantity" name="itemQuantity"  id="itemQuantity${i}" min="1" max="100" value=${data.nbr}>
            </div>
    ...
    if (TailleTab.length == affichage.length) { //Lorsque tout les "kanap" du LS sont affichés :
...
function totalDisplayOnClick(e) { //Récup (e) : où le "kanap" choisi est affiché avec son prix et sa quantitée
    let prixKanapTot = tabPrice[e]; // Récup. prix "kanap" du tableau de prix créé dans la fonction displayKanap()
    let itemQuantityKanap = Number(document.querySelector(`#itemQuantity${e}`).value); // Récup. de la nouvelle quantitée choisis
    document.querySelector(`#priceKanap${e}`).innerHTML = `${prixKanapTot * itemQuantityKanap} Euros`; //Nouveau prix Total pr un type de "kanap"
    document.querySelector("#totalPrice").innerHTML = 0 // init prix tot.  "kanap"
    for (j = 0; j < i; j++) { // re init prix total "kanap"
        document.querySelector("#totalPrice").innerHTML = Number(document.querySelector("#totalPrice").innerHTML) +
            (Number(document.querySelector(`#itemQuantity${j}`).value) * Number(tabPrice[j]));
    };

    document.querySelector("#totalQuantity").innerHTML = 0; // init. nbre total de "kanap"
    for (j = 0; j < i; j++) { // re init nbre total de "kanap"
        document.querySelector("#totalQuantity").innerHTML = Number(document.querySelector("#totalQuantity").innerHTML) +
...
function deleteItem(e) { //appel au "click" avec recup. de l'index "e" envoyé par "(${i})"
...
function dataUser() {
    ...
    if (nameRegex.test(firstName)) { //Comparaison avec regex
        allowSendFiN = true; //autorisation d'envoie à l'API
        document.querySelector("#firstNameErrorMsg").innerHTML = "";
    } else {
        document.querySelector("#firstNameErrorMsg").innerHTML = "Votre nom n'est pas écris correctement"; //rapport d'une erreur 
        allowSendFiN = false;
    ...
    if (nameRegex.test(lastName)) { //comparaison avec regex 
        document.querySelector("#lastNameErrorMsg").innerHTML = "";
        allowSendLN = true; //autorisation d'envoie à l'API
    } else {
        document.querySelector("#lastNameErrorMsg").innerHTML = "Votre prénom n'est pas écris correctement" //rapport d'une erreur 
        allowSendLN = false;
        allowSend = false; //refus d'envoie à l'API
    }
    /*address*/
    let address = document.querySelector("#address").value; //Adresse entré par l'utilisateur
    if (addressRegex.test(address)) { //comparaison avec regex 
        document.querySelector("#addressErrorMsg").innerHTML = "";
        allowSendA = true; //autorisation d'envoie à l'API
    } else {
        document.querySelector("#addressErrorMsg").innerHTML = "Votre adresse n'est pas écris correctement" //rapport d'une erreur 
        allowSendA = false;
        allowSend = false; //refus d'envoie à l'API
    }
    /*city*/
    let city = document.querySelector("#city").value; //Ville entré par l'utilisateur
    if (cityRegex.test(city)) { //comparaison avec regex 
        document.querySelector("#cityErrorMsg").innerHTML = "";
        allowSendC = true; //autorisation d'envoie à l'API
    } else {
        document.querySelector("#cityErrorMsg").innerHTML = "Le nom de votre ville n'est pas écris correctement" //rapport d'une erreur 
        allowSendC = false;
        allowSend = false; //refus d'envoie à l'API
    }
    /*email*/
    let email = document.querySelector("#email").value; //Email entré par l'utilisateur
    if (emailRegex.test(email)) { //comparaison avec regex 
        document.querySelector("#emailErrorMsg").innerHTML = "";
        allowSendE = true; //autorisation d'envoie à l'API
    } else {
        document.querySelector("#emailErrorMsg").innerHTML = "Votre email n'est pas écris correctement" //rapport d'une erreur 
        allowSendE = false;
        allowSend = false; //refus d'envoie à l'API
    }
    if ((allowSendFiN == false) || (allowSendLN == false) || (allowSendA == false) || (allowSendC == false) || (allowSendE == false)) {
        allowSend = false;
        alert("Vos coordonnés ne sont pas correctement remplie");
    } else {
        allowSend = true;
    }
};
...
async function validForm() {
    ...
    if ((allowSend == true) && (KanapVF != null)) { //Si l'autorisation d'envoyé vers l'API est valide et si présence "Kanap" dans LS
        sendData(); //Appel de la fonction qui envoie les données à l'API
    } else if ((allowSend == true) && (KanapVF == null)) { //Si pas de "Kanap" ds LS rapport d'erreur
...
function sendData() {
    ...
    for (let productsKanap of KanapSD) { // Boucle qui insert les id. de Kanap dans products
    ...
    const promise01 = fetch(urlOdrer, {
        ...
        headers: { //Informe API, données envoyées: "json"
            Accept: "application/json",
            "Content-Type": "application/json"
        }
    });
    /*Réception des données envoyé par l'Api*/
    promise01.then(async(response) => { //att. de la response de la promise01
        try {
...
function clientConfirm(numCom) {
    ...
    /*Création Id client*/
    const clientId = "?id=" + milliseconds + "" + seconds + "" + minutes + "" + hours +
        ...
        document.querySelector("#email").value.length + document.querySelector("#firstName").value.charAt(0) + document.querySelector("#lastName").value.charAt(0);
    localStorage.removeItem("Kanap"); // Clear LS
    localStorage.setItem("numCom", JSON.stringify(numCom)); //numéro de commande envoyé au LS
    let urlConfirm = new URL('http://127.0.0.1:5500/front/html/confirmation.html');
