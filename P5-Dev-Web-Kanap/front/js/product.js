...
window.onload = function getKanapApiDisplay() { //appel la fonction au chargement de la page
    ...
    fetch(`http://localhost:3000/api/products/${id}`) //requête "fetch" pour aller chercher les informations dans l'API avec l'id du "kanap" à afficher
        ...
        .then(function(data) { //Récupération de la promise en .json; data est un objet
            document.querySelector(".item__img").innerHTML = `<img src="${data.imageUrl}" alt="${data.altTxt}"/>`; //image vers product.html        document.querySelector("#title").innerHTML = `<h1 id="title">${data.name}</h1>`;
            document.querySelector("#price").innerHTML = `<span id="price">${data.price}</span>`; //prix vers product.html
            document.querySelector("#description").innerHTML = `<p id="description">${data.description}</p>`; //description vers product.html
...
function saveKanap(Kanap) { //Enregistre le panier dans le localStorage
...
function getKanapLS() { //fonction qui recupère ou non le panier situé dans le localStorage
    let Kanap = [];
    Kanap = localStorage.getItem("Kanap"); //On enregistre dans la variable Kanap les données recupérés du LocalStorage
    if (Kanap == null) { // si Le panier du localStorage est vide
        return []; //On return un tableau vide => panier vide
    } else { // sinon Le panier du localStorage existe
...
const addKanap = () => {
    if (document.querySelector("#colors").value == '') { // Empêche l'envoie au panier si pas de couleur séléctionné
        alert("Veuillez choisir une couleur de kanap"); //Rapporte une erreur
    } else if (document.querySelector("#quantity").value == 0) { // Empêche l'envoie au panier si pas de nombre séléctionné
        alert("Veuillez sélectionner un nombre d'article"); // Rapporte une erreur
    } else {
        ...
        const KanapPanier = Object.assign({}, kanapData, { //Création d'un nouvelle objet "kanaPanier",assignation,id,coul,nbr
            colors: `${selectcoul.value}`, //Récup. couleurs selectionné par l'utilisateur
            nbr: `${selectnbr.value}`, //Récup. nbr sélectionné par l'utilisateur
        });

        alert('Vous avez ajouter un Kanap'); // Préviens l'utilisateur de l'ajout d'un "kanap"
